...
local_data_path = (
    ...
    / ".."
    / ".."
    / "CMIP-GHG-Concentration-Generation/output-bundles/dev-test-run/data/processed/esgf-ready/input4MIPs"
)
...
for file in tqdm.tqdm(files_to_parse, desc="Extracting file metadata"):
    for source_id in source_id_drs_map:
        ...
        msg = f"No matching source ID found in {file!s}"
...
def load_cmip7_data(fps: list[Path]) -> xr.Dataset:
    ...
    out = out.rename(
        {k: v for k, v in CMIP7_TO_NORMAL_VARIABLE_MAP.items() if k in out.data_vars}
    )
...
wmo_ch7_df = wmo_ch7_df_raw.rename(
    {"Year": "year", **wmo_variable_normalisation_map}, axis="columns"
)
...
western_df_raw = pd.read_csv(
    PROCESSED_DATA_DIR / "western-et-al-2024" / "hcfc_projections.csv"
)
western_df = western_df_raw.rename(
    {"Year": "year", **wesetern_variable_normalisation_map}, axis="columns"
)
...
velders_df_raw = pd.read_csv(
    PROCESSED_DATA_DIR / "velders-et-al-2022" / "hfc_projections.csv"
)
velders_df = velders_df_raw.rename(
    {"Year": "year", **velders_variable_normalisation_map}, axis="columns"
)
...
adam_df_raw = pd.read_csv(
    PROCESSED_DATA_DIR / "adam-et-al-2024" / "hfc23_projections.csv"
)
...
for time_range in (
    ...
    for data_var in data_vars_to_plt:
        cmip_data = (
            ...
            .to_dataframe()
            .reset_index()
            .rename({"source_id": "source"}, axis="columns")
        )
        ...
        if data_var in wmo_ch7_df:
            pdf = pd.concat(
                [pdf, wmo_ch7_df[["year", "source", data_var]]]
        ...
        if data_var in western_df:
            pdf = pd.concat(
                [pdf, western_df[["year", "source", data_var]]]
            ).reset_index(drop=True)
        else:
        ...
        if data_var in velders_df:
            pdf = pd.concat(
                [pdf, velders_df[["year", "source", data_var]]]
            ).reset_index(drop=True)
        else:
        ...
        if data_var in droste_df_cg:
            pdf = pd.concat(
                [pdf, droste_df_cg[["year", "source", data_var]]]
            ).reset_index(drop=True)
        else:
        ...
        if data_var in droste_df_tal:
            pdf = pd.concat(
                [pdf, droste_df_tal[["year", "source", data_var]]]
            ).reset_index(drop=True)
        else:
            print(f"{data_var} not in Droste et al., 2022, Tacloneston")

        if data_var in adam_df:
            pdf = pd.concat([pdf, adam_df[["year", "source", data_var]]]).reset_index(
                drop=True
            )
        else:
...
for data_var in sorted(loaded.data_vars):
    ...
    difference = parray.sel(source_id=CMIP7_COMPARE_SOURCE_ID) - parray.sel(
...
for data_var in sorted(loaded.data_vars):
    ...
    fig, axes = plt.subplot_mosaic(
        [["recent", "recent"], ["all", "historical"]], figsize=(10, 6)
    )

    for time_axis, ax in (
        (slice(None, None), "all"),
        (slice(1950, None), "recent"),
        (slice(1750, None), "historical"),
    ):
        parray.sel(year=time_axis).plot.line(
            x="year", hue="source_id", linewidth=3, alpha=0.4, ax=axes[ax]
        )
    ...
    try:
        ...
        for time_axis, ax in (
            ...
            difference_rf.sel(year=time_axis).plot.line(
                x="year", hue="source_id", alpha=0.9, ax=axes[ax]
            )

        fig.suptitle(
            f"{parray.name} ERF difference ({CMIP7_COMPARE_SOURCE_ID} - {CMIP6_SOURCE_ID})"
        )
    ...
    for time_axis, ax in (
        ...
        difference.sel(year=time_axis).plot.line(
            x="year", hue="source_id", alpha=0.9, ax=axes[ax]
        )

    fig.suptitle(
        f"{parray.name} absolute difference ({CMIP7_COMPARE_SOURCE_ID} - {CMIP6_SOURCE_ID})"
    )
    ...
    difference_rel = (
        (
            parray.sel(source_id=CMIP7_COMPARE_SOURCE_ID)
            - parray.sel(source_id=CMIP6_SOURCE_ID)
        )
        / parray.sel(source_id=CMIP7_COMPARE_SOURCE_ID)
        * 100
    )
    ...
    for time_axis, ax in (
        ...
        difference_rel.sel(year=time_axis).plot.line(
            x="year", hue="source_id", alpha=0.9, ax=axes[ax]
        )
        axes[ax].set_ylim([-10, 10])

    fig.suptitle(
        f"{parray.name} percentage difference ({CMIP7_COMPARE_SOURCE_ID} - {CMIP6_SOURCE_ID})"
    )
